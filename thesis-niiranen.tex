...
especially favoured in telecommunication systems due to its facilities for concurrency and messaging.
...
The thesis utilizes literature review as its research method. The studies and metrics presented are chosen by their apparent influence in the field, mainly measured by their citation count in Google Scholar~\parencite{kousha2007google}. Moreover, iterating sources, or~\textit{snowballing} as it is called in research, is used to build an intact and chronologically sound research body for the thesis. The research methodology is explained in-depth in the last chapter (Summary).
...
Haskell is considered to be a statically typed language with strong type inference. However, programming language type systems and type theory involve varying perspectives and terms, to which no unanimous consent exists. To exaggerate the overlapping of different type concepts, a pure functional language such as Haskell also exhibits characteristics of dynamic checking and both implicit and explicit typing~\parencite{hudak2007history}.
...
The validation methodology used by~\citet{ryder2004software} is based on~\textit{subjective complexity}, which is referred as ``a notion of the complexity that might be perceived by a programmer attempting to understand or modify the given function''~\parencite[103.]{ryder2004software}. The metrics are applied to source code version histories of two relatively small programs and statistical analysis is utilized for determining the correlation of the measurements.
...
First recursion measure provided by~\citet{ryder2004software} is the~\textit{binary indication of recursion}. The metric simply determines if a function is recursive in nature, indicated by a value of zero or one. This seemingly simple metric can correlate with the amount of changes in the code base, and thus may indicate increased subjective complexity. Non-trivial recursion is when a function invokes a function, which in turn invokes the original function back. This kind of
...
The positive side is that the functional paradigm seems to be well-suited for software measurement. Metrics such as~\textit{number of constructors} and~\textit{number of overridden pattern variables}~\parencite{ryder2004software} are easy to be quantified and show promising preliminary results. Call graphs are convenient for visual comprehension of the program logic flow, but arguably even more so for functional programs where
...
The research method chosen for the thesis - literature review - is fitting, since the subject is variably researched~\parencite{kitchenham2010s,ryder2005software} and conducting reviews is an excellent way of mapping the current situation and generating interest in to the subject. The literature for this thesis was searched with Google Scholar (see~\citet{kousha2007google}) and was chosen based on (1) the influence on other research (citation count) and by
(2) pruning (``snowballing'') these citation lists for studies relevant to the topic. Sources were searched with the keywords ``software metrics'', ``functional programming'' and the previous joined together. The process of gathering the references had no fixed timespan, but rather was active for almost the life time of the thesis writing process from early 2014 to late 2014.
...
The results of the literature review are promising. Withstanding the lack of studies in the early 80s and low activity in the 00s, software metrics research has shown credible activity during the recent years. Moreover, interest in functional programming is rising, which is evident from new studies measuring complexity of functional programs (see~\citet{kiraly2010implementing,muddu2013metrics,kiraly2013complexity}). On the other hand, much of the
