...
Functional languages utilize either~\textit{strict} or~\textit{non-strict} semantics. These are derived from evaluation strategies of lambda calculus expressions, where the order of reducing the expression variables are dictated. In the~\textit{normal order} strategy, a lambda calculation is reduced as follows:
...
Thus the leftmost, outermost expression is always reduced first~\parencite{pierce2002types}. The~\textit{call-by-name} strategy, on the other hand, only partially reduce expressions, not resulting to actual values: 
...
This has the advantage that computations are only done when they are needed. For example, Haskell utilizes an optimized version of this strategy, where expressions once evaluated are saved and the values can reused without repeating the computations. Such semantics are called non-strict, where~\textit{lazy evaluation} is one implementation technique and utilized by e.g. Haskell.~\parenciteseveral{hudak2007history}.

The strict semantics are implemented most of the time through~\textit{call-by-value} strategy, where only the outermost expression is evaluated and ``only when its right-hand side has been reduced to a value''~\parencite{pierce2002types}[57.]:
...
This approach creates the strict semantics, because function parameters are always evaluated even when they are not used~\parencite{pierce2002types}.

~\parencite{pierce2002types}[57.] juttua evaluation strategyista
