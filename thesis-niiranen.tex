...
The conceptual distance between two interacting functions is an interesting property to measure. It is likely that if the distance is large, also the source code is harder to be comprehended and the probability of errors in code increase. However, distance can be interpreted in multiple ways. First, the~\textit{number of scopes} that are introduced for a executed function call can be quantified.
...
\noindent Example~\ref{lst:number-of-scopes} contains the scopes {\tt foo} and {\tt bar}. In addition, {\tt a b}, {\tt fn c}, and {\tt x,y} are considered as scopes for the function {\tt foo} when it is called.
