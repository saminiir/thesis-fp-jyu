...
Functional programming, albeit sufficiently researched, is mostly an interest in academia. Nevertheless, the research of the paradigm is active, starting from LISP~\parencite{backus1978can} to the current research of Haskell being the most vigorously researched functional programming language~\parencite{wadler1995monads,hudak2007history}. In the industry side, Erlang~\parencite{armstrong1993concurrent} is an example of a heavily-used functional programming language. It is
especially favoured in telecommunication systems due to its facilities for concurrency and messaging.

Previous research combining software metrics and functional programming is much lower in quantity when comparing to the imperative and object-oriented paradigms. The basis for the research is done by~\citet{van1995software}, who assessed the complexity of code produced by students participating in functional programming courses.~\citet{ryder2004software} presented more metrics by conducting in-depth research and also proposed a software tool for calculating the
result of several metrics in functional programs. In more recent years, research is concentrating on programming languages such as Scala and the module complexity measurement in open-source software~\parencite{muddu2013metrics}.

As code complexity is an inherent problem of software development, it is only natural that possible solutions have been researched since the first computer systems(TODO: Lahde?). Functional programming is touted as addressing the complexity issue, but large-scale validation in industry-scale systems has not been viable due to lack of adoption of the functional paradigm. In specific, commercial use heavily favours the object-oriented and imperative paradigms. Although software metrics
have been extensively researched, the amount of studies specific to the functional paradigm is still low in quantity. Thus the need for further study is warranted, if only to motivate the research field and scope new directions. 
...
The thesis utilizes literature review as its research method. The review starts with an overview into software metrics. In particular, the earliest software metrics are presented to provide a basis for more recent studies. The newer research is presented and possible challenges are discussed. In the end of every chapter is a summary composing the content.

The studies and metrics presented are chosen by their apparent influence in the field, dictated by their citation count in Google Scholar~\parencite{kousha2007google}. 

The results aim at explaining the various semantics of FP and how they relate to software quality metrics. The research is expected to be of significance to researchers and practitioners alike. The results will help in formulating software metrics and functional programming into practical settings. Future research will be fueled by providing more support to the functional paradigm and by encouraing real-world usage of the functional paradigm.
