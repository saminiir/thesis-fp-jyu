...
\noindent This approach enforces the strict semantics of evaluation, because function parameters are always eagerly evaluated even when they are not used~\parencite{pierce2002types}. 

Non-strict evaluation strategies, however, have the benefit of not evaluating computations until needed. For example, in the~\textit{normal order} strategy, a lambda expression is reduced as follows:
...
\noindent Thus the leftmost, outermost expression is always reduced first~\parencite{pierce2002types}, causing functions to be applied before evaluating their arguments. However, the normal-order reduction is potentially inefficient when dealing with nontrivial computations, leading to unnecessary repetitive calculations in naive implementations~\parencite{hudak1989conception}. The~\textit{call-by-name} strategy, on the other hand, only partially reduces expressions, not resulting to actual values: 
