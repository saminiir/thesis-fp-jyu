...
Polymorphism is further divided into more detailed concepts.~\textit{Parametric} polymorphism enables functions to operate on a uniform set of unconstrained types that share a common structure~\parencite{cardelli1985understanding}. For example, the idiomatic function {\tt id :: a -> a} generally accepts any kind of type as its argument and as a result, returns a value of the same unknown type.~\textit{Ad-hoc} polymorphism, on the other hand, enables functions to operate on seemingly unrelated types with an implementation that is specific to the type~\parencite{cardelli1985understanding}. The addition operator {\tt +} is ad-hoc in many languages, since it operates differently based on its input types. For example, adding two strings together might result in a concatenated string, where as adding two decimal numbers might utilize a floating point operation to effectively calculate the result. In fact, the types have~\textit{overloaded} the implementation of the addition
operator. 

An essential part of modern functional languages are~\textit{data types}. Furthermore,~\textit{pattern matching} is often utilized to enable succinct logic branches in functional programming. The implementation of these subjects is not a trivial matter, however, and has required some compromises in the Haskell language.~\parenciteseveral{hudak2007history}. 
%{{{
\begin{comment}
