...
\noindent These variables are then used to quantify several properties such as the~\textit{length},~\textit{vocabulary},~\textit{volume},~\textit{program level} and~\textit{difficulty} of the source code. The example listing~\ref{lst:halstead-example} of C code has thus 4 operands ({\tt a, b, sum, "sum of.."}) and 4 operators ({\tt int, =, +, printf}).
...
The Halstead program volume measures are calculated for listing~\ref{lst:halstead-example} as follows. The length of listing~\ref{lst:halstead-example} is 14 ($N_1 + N_2$). The vocabulary is calculated with $\mu_1 + \mu_2$, which amounts to 8. 

(TODO: Calculations and examples of the formulas)
...
~\citet{sarkar2007api} measure the quality of modularization 
~\parencite{sarkar2007api}
~\parencite{beck2011congruence}
