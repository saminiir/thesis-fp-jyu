...
An example of spatial distance is to measure the actual source code lines from a function call to its declaration. In essence, the call to function {\tt foo} at line 8 (figure~\ref{lst:number-of-scopes}) is 7 source code lines away from its declaration (if empty lines are counted). The empirical results of~\citet{ryder2004software} leaves room for further research, since mixed results from the measure were found. 

Counting the distance based on source lines can be argued as being easily misinterpreted, since source lines can e.g. contain multiple statements. For this reason, the~\textit{number of parse tree nodes} can be a more accurate spatial distance measure. As program code is often internally represented in parse trees, it makes sense to count the amount of nodes leading from a declaration to its usage. This approach has the advantage that the actual logic is only dealt with and the
...
recursion produces a cyclic callgraph and is harder to detect in the source code. In addition, a function can contain both trivial recursion (the function invokes itself) or non-trivial recursion at the same time. For this case, the binary indication of recursion simply points out whether a function is recursive without pointing out the implementation.
