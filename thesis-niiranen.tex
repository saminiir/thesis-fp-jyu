...
\noindent This has the advantage that computations are only evaluated when they are needed. Furthermore, Haskell utilizes an optimized, albeit a more complex version of this strategy called \textit{call-by-need}. In call-by-need, expressions once evaluated are saved and the values are reused without repeating the computations. This~\textit{lazy evaluation} is achieved by through graph reduction, where arguments and their evaluated results are shared between computations~\parencite{hudak2007history}. 
