...
\abstract{
Software metrics have long been utilized for quantifying the quality of code and for directing maintenance efforts to problematic program modules. The functional programming paradigm provides an alternative approach where program code is structured with notions from mathematics. This thesis investigates the current status of software metrics and how they apply to the functional paradigm. The research method utilized is literature review and the sources were gathered by conducting
keyword-based searchs and choosing popular studies based on their citation count in Google Scholar.\\
\\
The findings of the thesis are that software metrics research is alive, but has gone through many transformations over the decades. Furthermore, software metrics research for the functional paradigm is somewhat scarce, but robust and replicable. The research needs further validation with larger systems. Future research is deemed to consist of combining the positives of software metrics, functional programming and the current agile way of software engineering.
}
\tiivistelma{
Ohjelmistometriikoita on pitkään käytetty mittaamaan ohjelmistojen koodin laatua ja ylläpidon ohjaamiseen ongelmallisiin komponentteihin. Funktionaalinen ohjelmointiparadigma tarjoaa lähestymistavan, missä ohjelmakoodi strukturoidaan matemaattisten käsitteiden mukaisesti. Tämä tutkielma kartoittaa ohjelmistometriikoiden nykytilan ja tutkii kuinka niitä on käytetty funktionaalisessa paradigmassa. Tutkimusmenetelmänä on kirjallisuuskatsaus. Lähteet haettiin Google Scholarista
avainsanojen perusteella ja valitsemalla tutkimukset niiden suosion (sitaattien määrän) ja julkaisun ajankohdan perusteella.\\
\\
Tutkielman tutkimustulokset osoittavat, että ohjelmistometriikan tutkimus on eloisaa monesta suunnanvaihdoksesta huolimatta. Funktionaalisen ohjelmoinnin metriikkatutkimus on niukkaa, mutta laadultaan hyvää ja toistettavissa olevaa. Tutkimus tarvitsee kuitenkin lisävalidaatiota isompien järjestelmien kanssa. Jatkotutkimusaiheena ehdotetaan ohjelmistometriikoiden ja funktionaalisen ohjelmoinnin positiivisten puolten hyödyntämistä ketterässä ohjelmistokehityksessä.  
}

\avainsanat{tietojärjestelmätiede, pro gradu -tutkielma, ohjelmistometriikat, funktionaalinen ohjelmointi} 
\keywords{information technology, Master's thesis, software metrics, functional programming} % avainsanoilla
