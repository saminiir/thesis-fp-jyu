...
Previous research combining software metrics and functional programming is much lower in quantity when comparing to the imperative and object-oriented paradigms. The basis for the research is done by~\citet{van1995software}, who assessed the complexity of code produced by students participating in functional programming courses.~\citet{ryder2004software} presents more metrics by conducting in-depth research and also proposes a software tool for calculating the
result of several metrics in functional programs. In the recent years, research is concentrating on programming languages such as Scala and the measurement of module complexity in open-source software~\parencite{muddu2013metrics}.

As code complexity is an inherent problem of software development, it is only natural that possible solutions have been researched since the first computer systems(TODO: Lahde?). Functional programming is touted as addressing the complexity issue, but large-scale validation in industry-scale systems has not been viable due to lack of adoption of the paradigm. In other words, commercial use heavily favours the object-oriented and imperative paradigms(TODO: Lahde). Although software metrics
have been extensively researched, the amount of studies specific to the functional paradigm is still low in quantity. Thus the need for further study is warranted, if only to motivate the research field and to scope new directions. 
...
pattern variables). Reproducibility also seems to be taken into account, since the research methods are clearly explained and should pose no restrictions to further replication of the results.
