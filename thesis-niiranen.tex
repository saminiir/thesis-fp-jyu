...
A dominant type system in functional programming is the Hindley-Milner (HM) type system~\parencite{milner1978theory}, where~\textit{type inference} plays an important role. Type inference enables automatic deducing of the types in expressions, without explicit declaration. This frees developers from having to repeat type declarations, but still offers polymorphic functions and data structures with accurate type information. Indeed, the ML language, a major inspiration of Haskell, was the first statically typed language to integrate type inference into its implementation~\parencite{hudak1989conception}.
