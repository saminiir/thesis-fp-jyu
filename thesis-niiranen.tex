...
~\parencite{muddu2013metrics}

\subsection{Metrics for Higher-order functions}

The use of higher-order functions is prevalent in functional programming. Higher-order functions are functions that receive functions as parameters and can output a function as their return value. The primary benefit of higher-order functions is perhaps in modularization, since e.g. data structures can be traversed by higher-order functions without explicitly declaring how the data is utilized~\parencite{muddu2013metrics}.
%{{{
\begin{comment}
...
Slightly confusingly,~\citet{muddu2013metrics} define several metrics for ``first order'' functions, even though the more prevalent term seems to be higher-orderism.~\citet{muddu2013metrics} argue that if first-order functions are separated in different modules, additional dependencies are created and thus program modularization might suffer.
