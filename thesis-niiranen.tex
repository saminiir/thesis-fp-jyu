...
\translatedtitle{Ohjelmistometriikat ja funktionaalinen ohjelmointi}

\author{Sami Niiranen}
\contactinformation{sami.i.niiranen@student.jyu.fi}

\supervisor{Kaijanaho, Antti-Juhani}
\supervisor{Tirronen, Ville}
\supervisor{Veijalainen, Jari}

\setdate{02}{03}{2014}

\abstract{
Software metrics have long been utilized for quantifying the quality of code and for directing maintenance efforts to problematic program modules. The functional programming paradigm provides an approach where program code is structured with notions from mathematics. This thesis investigated the current status of software metrics and how they apply to the functional paradigm. The research method utilized was literature review and the sources were gathered by conducting keyword-based searchs and
by choosing popular studies based on their citation count in Google Scholar. The findings of the thesis were that software metrics research is alive, but has gone through many transformations over the decades. Furthermore, software metrics research for the functional paradigm is somewhat scarce, but robust and replicable. The field's research needs further validation with larger systems. Future research is proposed to consist of combining the positives of software metrics, functional programming and the current agile way of software engineering.
}
\tiivistelma{
Ohjelmistometriikoita on pitkään käytetty mittaamaan ohjelmistojen koodin laatua ja ylläpidon ohjaamiseen ongelmallisiin komponentteihin. Funktionaalinen ohjelmointiparadigma tarjoaa lähestymistavan, missä ohjelmakoodi strukturoidaan matemaattisten käsitteiden mukaisesti. Tässä tutkielmassa kartoitettiin ohjelmistometrikoiden nykytilaa ja tutkittiin kuinka niitä on käytetty funktionaalisessa ohjelmointiparadigmassa. Tutkimusmenetelmänä oli kirjallisuuskatsaus. Lähteet haettiin Google Scholarista avainsanojen perusteella ja valitsemalla tutkimukset niiden suosion (sitaattien määrän) ja julkaisun ajankohdan perusteella. Tutkielman tutkimustulokset osoittivat, että ohjelmistometriikan tutkimus on eloisaa monesta suunnanvaihdoksesta huolimatta. Funktionaalisen ohjelmoinnin metriikkatutkimus on niukkaa, mutta laadultaan hyvää ja toistettavissa olevaa. Alan tutkimus tarvitsee kuitenkin lisävalidaatiota isompien järjestelmien kanssa. Jatkotutkimusaiheena ehdotetaan ohjelmistometriikoiden ja funktionaalisen ohjelmoinnin positiivisten puolten hyödyntämistä ketterässä ohjelmistokehityksessä.  
}

\avainsanat{ohjelmistosuunnittelu, ohjelmistometriikat, funktionaalinen ohjelmointi, Haskell} 
\keywords{software design, software metrics, functional programming, Haskell} % avainsanoilla
...
\thispagestyle{empty}
Software quality metrics is a branch of software engineering where the quality of code is analyzed and quantified. The purpose of this observation is to identify problematic modules in the code and predict future maintenance effort~\parencite{fenton1999software}. This need is evident in any nontrivial software system, where further developing the program modules often result in increased code complexity. NASA, for example, uses software metrics as an aid to validate their spaceflight systems~\parencite{judas2011historical}.

Several terms and interpretations exist for measuring the quality of software. The definition for software quality metrics is given in the IEEE standard 1061~\parencite{ieeesoftwaremetrics1998} as: 
~\begin{quoting}
...
This thesis concentrates on examining the code-specific attributes of software, rather than external attributes affecting software quality such as management decisions. A classical example of a software metric is the lines of code (LOC) metric, where the input (according to IEEE standard 1061 for software quality metrics) is the source code of the program, and the output is the number of source code lines. Then interpretations on the software's quality can be proposed. For example, a program
module with a high LOC count can be argued to be more complex than a module with fewer lines.~\citet{fenton1998software} regard such measures as structural and complexity metrics. Other classic examples of software metrics are cyclomatic complexity~\parencite{mccabe1976complexity} and Halstead's metric suite~\parencite{halstead1977elements}. This thesis uses the shortened terms,~\textit{software metrics} and~\textit{software measurement}, to describe software quality metrics as defined by the IEEE standard 1061.
%{{{
\begin{comment}
...
Functional programming aims for program code that is succinct and modular. In specific, source code written in a functional language often consists of several small utility functions that can be combined together in an extensible manner~\parencite{hughes1989functional}. This is achieved through mechanisms such as higher-order functions and algebraic data types. The development of functional programs is aided by features from mathematics.~\textit{Referential
transparency} guarantees that expressions return the same output with the same input, resulting in easier reasoning of the program logic. Computational contexts such as~\textit{functors} provide clean abstractions, for example, to error values. State, manifesting itself often as global variables in imperative languages such as C, is avoided and instead, immutable data structures are favoured.~\parenciteseveral{wadler1992essence}.  

As the popularity of functional programming is increasing with languages such as Scala~\parencite{odersky2004overview} and Haskell~\parencite{marlow2010haskell}, also researching the applicability of software metrics to the functional paradigm is intriguing. Since both fields of research advocate maintainable systems, combining software metrics and functional programming can be argued to result in improved software quality.  

Previous research in software metrics is numerous, starting from the classic works of~\citet{mccabe1976complexity,halstead1977elements}. In the 90s, metrics research achieved its height due to the popularity of the object-oriented paradigm~\parencite{lorenz1994object,briand1996property,hitz1996chidamber,fenton1998software}, but has continued to be researched in a steady fashion ever since~\parencite{olague2007empirical,gui2008new,ordonez2008state}. Several
reviews and mapping studies have also been conducted~\parencite{kitchenham2010s,tahir2012systematic} that accurately compose the positive and the negative sides of past software metrics research.

Functional programming, albeit sufficiently researched, is mostly an interest in academia. Nevertheless, the research of the paradigm is active, starting from LISP~\parencite{backus1978can} to the current research of Haskell being the most vigorously researched functional programming language~\parencite{wadler1995monads,hudak2007history}. At the industry side, Erlang~\parencite{armstrong1993concurrent} is an example of a heavily-used functional programming language. It is especially favoured in telecommunication systems due to its facilities for concurrency and messaging.

Previous research combining software metrics and functional programming is much lower in quantity when comparing to the imperative and object-oriented paradigms. The basis for the research is done by~\citet{van1995software}, who assessed the complexity of code produced by students participating in functional programming courses.~\citet{ryder2004software} presents more metrics by conducting in-depth research and also proposes a software tool for calculating the result of several metrics in functional programs. In the recent years, research is concentrating on programming languages with facilities for functional programming and the measurement of module complexity in open-source software~\parencite{muddu2013metrics}.

As code complexity is an inherent problem of software development, it is only natural that possible solutions have been researched since the first computer systems. Functional programming is touted as addressing the complexity issue~\parencite{backus1978can,hughes1989functional}, but large-scale validation in industry-scale systems has not been viable due to lack of adoption of the paradigm. In other words, commercial use heavily favours the object-oriented and imperative paradigms. Although software metrics have been extensively researched, the amount of studies specific to the functional paradigm is still low in quantity. Thus the need for further study is warranted, if only to motivate the research field and to scope new directions. 

The aim of this thesis is to provide an up-to-date overview of software metrics and discuss possible future research directions. Moreover, metrics are specifically pruned in the context of functional programming languages, where Haskell is used as reference. As an example,~\citet{ryder2004software} encourages future research to measure polymorphism and higher-order functions in functional programming.

The research can be decomposed into four questions:

\begin{itemize}
    \item What are software metrics?
    \item What is functional programming?
    \item How do existing software metrics apply to the functional paradigm? 
    \item What is the status of software metrics research for the functional paradigm?
\end{itemize}

\noindent The research problem can be expressed as follows:~\textit{What kind of software metrics exists for the functional programming paradigm, and how do the metrics apply to them?}

The thesis utilizes literature review as its research method. The literature for this thesis was searched with Google Scholar (see~\citet{kousha2007google}) and was chosen based on (1) the influence on other research (citation count) and by (2) pruning (``snowballing'') these citation lists for studies relevant to the topic. Sources were searched with the keywords ``software metrics'', ``functional programming'' and the previous joined together. The process of gathering the references had no fixed timespan, but rather was active for almost the life time of the thesis writing process from early 2014 to late 2014.

The selection of the references was also influenced by the age of the research. New research, such as~\citet{beck2011congruence,muddu2013metrics}, was deemed to be interesting from the standpoint of literature review. On the other hand, plenty popular, albeit old, research exists~\parencite{mccabe1976complexity,backus1978can,halstead1977elements} and were included in the thesis for wholeness and their field-defining content. Nevertheless, the thesis aims to provide an extensive investigation of the field of software metrics spanning many decades.

The scientific contribution of the thesis is to explain various semantics of functional programming and review the current status of software metrics and especially research that utilizes functional programming. The research is expected to be of significance to researchers and practitioners alike, mainly by formulating software metrics and functional programming research into a comprehensive overview. Future research will be fueled by providing more support to the functional
paradigm and by encouraging improving the maintainability of systems through measurement.

As previously stated, this thesis adopts software measurement as defined in the IEEE standard 1061 for software quality metrics. Moreover, algorithmic efficiency can be thought of as a ``internal attribute'' of code, but such performance-related measurement is out of the scope of the thesis. Code complexity is the measurement attribute this thesis is mainly covering. The thesis also recognizes the inherent subjectivity of a qualitative attribute such as code complexity. This is,
of course, the ultimate reason quantitative approaches, such as software metrics, are devised.

The thesis consists of the following content: Chapter~\ref{chap:software_metrics} presents an overview on software metrics and chapter~\ref{chap:functional_programming} introduces functional programming. Chapter~\ref{chap:metrics_for_functional_programming} combines the two subjects, presenting software metrics research specific to the functional paradigm. Finally, chapter~\ref{chap:summary} concludes the literature review, presenting the findings and discussing future research possibilities.
...
First attempts at recording software metrics were conducted in the 1960s when properties such as program size were measured. The results were then used to assess programmer productivity and the quality of their work. In the 1970s the limitations of simple measures like lines of code were noted and research departed to produce more elegant measures. These new measures, such as the ones from~\citet{halstead1977elements} and~\citet{mccabe1976complexity} were aimed to provide complexity measures independent of the accompanying programming languages. Ever since, research has produced numerous new metrics and validation frameworks, each with their own advantages and disadvantages.~\parenciteseveral{fenton1999software}.

The definition for a software quality metric is given in IEEE standard 1061~\parencite{ieeesoftwaremetrics1998} as: 
~\begin{quoting}
A function whose inputs are software data and whose output is a single numerical value that can be interpreted as the degree to which software possesses a given attribute that affects its quality.
~\end{quoting}
...
This chapter continues on presenting many, if not all, classic measures of software code. The foundations are important for understanding the implications of the empirical research that has been conducted and nature of the trends in the field. After the initial presentation, the focus of the chapter shifts to newer measurement approaches such as the measurement of code version history, which, while not directly falling into the category of internal code attribute measurement, is a prevalent current trend in the field~\parencite{zimmermann2005mining}. Lastly, the challenges of metrics are discussed and the main obstacles of future research are provided.
...
Perhaps the first approach to measuring the internal attributes of software is the ~\textit{size} metric. It is relatively easy to quantify, since only source code is required for the analysis and the measurement is often made statically, meaning that the program code is not executed.~\citet{fenton1998software} divide the size metric into the following subcategories: 
...
The meaning of Halstead's mathematical models on real-world usage are unclear. Halstead is critized for not indicating whether the metrics are measures or predictive variables of systems.~\parenciteseveral{fenton1998software}. This might add to observations made by~\citet{jones1994software} about the convoluted nature of the Halstead metric and its inherent flawness in attempting to deliver accurate estimations of development and maintenance effort.
...
\begin{figure}[h]\centering
    ...
    \end{tikzpicture}
  \caption[Example of a flow graph~\parencite{allen1970control}
  ]{Example of a flow graph~\parencite{allen1970control}}
...
~\citet{ince1988approach} argue, that graph impurity should not be achieved at the cost of generally accepted design patterns. For example, a module that is often referenced by other modules would be considered as contributing to graph impurity, but the structure of the program can actually be well-designed because of module reuse. As is, graph impurity should not be interpreted fully objectively, but rather ``where choice exists a design should be produced which minimises graph
impurity''~\parencite[54.]{ince1988approach}.
%{{{
\begin{comment}
...
{\itshape Internal reuse} is the measure of to which extent a program reuses its own modules~\parencite{fenton1998software}.~\citet{yin1978establishment} define a measure for internal reuse ({\itshape system design measure}~\parencite{yin1978establishment}), which can be calculated from the call graph of a program:
%{{{
\begin{comment}
...
The definition by~\citet{stevens1974structured} consists of seven degrees of cohesion, listed from the most desirable to least desirable:
...
The information flow is relatively easy to determine, as it can be done by simply analyzing the procedures and calls of the system's modules~\parencite{henry1981software}. The~\textit{Henry-Kafura} measure~\parencite{henry1981software} formulates additional variables that aid in assessing the complexity of a module: The~\textit{fan-in} is the number of~\textit{local flows} into the module plus the number of data structures the module utilizes for information retrieval. On the other
hand,~\textit{fan-out} is the number of local flows out from a module plus the number of data structures the module updates. ~\parenciteseveral{henry1981software}.
%{{{
\begin{comment}
...
The Henry-Kafura measure is improved upon by the research of~\citet{ince1988approach}. Namely,~\citet{ince1988approach} address ``several theoretical problems with the measure''~\parencite{fenton1998software}, such as the informal notion of indirect flow and its applicability as a measurement model. In conclusion,~\citet{ince1988approach} propose an improved measure where the length of the module is not taken into consideration.
...
The~\textit{number of children} metric is likewise easy to measure. It is simply the number of immediate subclasses (children) of a class. The number of children metric is based on the notions that the greater the number of subclasses, also the greater the reusage of the parent class's methods is. With increased subclass count, however, also the likelyhood of misuse of inheritance by improper abstractions is increased. Lastly, the number of subclasses predict the impact of the parent
...
The metric~\textit{coupling between object classes} (CBO) is measured by the number of objects an object is coupled to.~\citet{chidamber1994metrics} define coupling as ``an object is coupled to another object if one of them acts on the other, i.e., methods of one use methods or instance variables of another''.  
%{{{
\begin{comment}
...
The~\textit{lack of cohesion in methods} (LCOM) metric measures the level of cohesion in an object class. Namely, it calculates the number of method pairs in a class that have no relation to each other. For example, if two methods do not share the use of an instance attribute of the object, their LCOM measure is zero. In essence, the LCOM metric ``provides a measure of the relative disparate nature of methods in the class''~\parencite[489.]{chidamber1994metrics}.
%{{{
\begin{comment}
...
An often cited principle of system design is~\textit{Conway's law}, which includes the observation that ``organizations which design systems (used in the broad sense here) are constrained to produce designs which are copies of the communication structures of these organizations''~\parencite[31.]{conway1968committees}. Although not a direct principle of software engineering, the notion of Conway's law has implications for the modularization degree and quality in software.

~\citet{beck2011congruence} provide several metrics for assessing the level of coupling and quality of a software's modularity. The metric~\textit{structural dependencies} is formed in three ways: two classes are coupled if they are (1) related by inheritance, (2) related by aggregate class variables used by another class, (3) related by direct usage, e.g. as a local variable or as a function parameter.
...
~\textit{Evolutionary coupling}~\parencite{beck2011congruence} occurs when two seemingly unrelated entities are often changed simultaneously during development. The metric~\textit{support} quantifies the number of common changes between the two entities.~\textit{Confidence} normalizes the support-metric by the total number of changes of one of the artifacts.
...
The field of software measurement has existed as a research discipline since the first computer systems have been developed. In this chapter, many classic static code quality metrics were presented and the current trends were analyzed. Metrics specific to the object-oriented programming paradigm dominate in popularity, but researchers are also turning to other aids such as mining data out of version control histories~\parencite{menzies2007data}. This proves that the field is still healthy and
new research topics are undoubtedly to be discovered. 
...
A recurring challenge of software engineering is to develop applications that are not complex to comprehend and reason about. Actions towards this complexity are actively being taken by e.g. through refactoring~\parencite{fowler1999refactoring}, and by analyzing the quality of the software with metrics~\parencite{boehm1976quantitative}. In turn,~\textit{modularization} is in the heart of software engineering, where the aim is to have loosely-coupled relationships between components and the possibility of changing and upgrading these components with least effort possible~\parencite{hughes1989functional}. Effectively, software that has a well-thought structure is easier to develop, debug and maintain.
%{{{
\begin{comment}
...
Several programming paradigms exist for developing software. The most popular paradigm is the~\textit{imperative} approach, where programs are written in step-by-step instructions and commands~\parencite{sebesta2002concepts}. This approach is similar to the design of the von Neumann computer architecture, where execution is implemented as sequential steps of instructions that manipulate the state of the  central processing unit and the memory~\parencite{backus1978can}. In turn, many programming languages (~\textit{C},~\textit{Java}) have adapted the same approach as in the von Neumann architecture to their execution flow: variables, control statements and assignment statements~\parencite{backus1978can}.
%{{{
\begin{comment}
...
Functional programming is a different approach to software engineering compared to imperative languages. It utilizes mathematical functions for program flow, such as recursion and conditional expressions, in contrast to the characteristic sequential and iterative repetition found in imperative languages~\parencite{sebesta2002concepts}. Moreover, variables that model values in memory locations do not exist in mathematics, and as such, are also omitted from pure functional programming. 
%{{{
\begin{comment}
...
The main motivation behind the functional programming paradigm is the use of mathematical functions in expressing programs concisely. Unlike in imperative languages such as C++, functions in functional languages resemble the mathematical notion of functions. That is, rather than specifying values over sequential operations, function parameters are meant to be mapped over values.~\parenciteseveral{hughes1989functional}. A function that returns a number doubled by itself (effectively calculating the square of a number) would be defined in Haskell as:
...
A particular inspiration for functional languages is the~\textit{lamdba calculus}~\parencite{church1932set}, where computation consists solely of function definition and application. The structure of the lambda calculus consists of three~\textit{lambda-terms}: variable {\tt x}, abstraction {\tt $\lambda x.t$} and application {\tt t t}. The basis of the calculus is simple and may be the reason for its popularity since the 1960s.~\parenciteseveral{pierce2002types}.

The application of lambda calculus is done by applying functions to arguments that are also functions. In specific, a~\textit{redex} (reducible expression) might be of the form {\tt ($\lambda x. x$) y}, which would after reduction be evaluated into {\tt y}. In essence, each computation consists of rewriting the left-hand side lambda-abstraction (e.g. {\tt ($\lambda x. x$))} with the right-hand side variable ({\tt y}) to the abstraction's bound variable of {\tt x}.~\parenciteseveral{pierce2002types}. 

The functions in lamdba calculus have one key abstraction: They can be applied to themselves. Because of this self-applicability and anonymity of functions, also the facilities for recursion are enhanced. In specific, recursion can be executed in a more direct manner ``without explicitly writing a recursive definition''~\parencite[363.]{hudak1989conception}. For an example of lamdba calculus, the following expression results in 8 as the value of the computation:

\begin{lstlisting}[mathescape]
$(\lambda (x)x * x * x)(2)$
\end{lstlisting}

\noindent Even though numerous calculi exist, Church's lambda calculus is the grand source for the computational aspects of functional languages~\parencite{hudak1989conception}. Indeed, many variants of lambda calculus exist with a corresponding amount of theories related to it.
%{{{
\begin{comment}
...
Higher-order functions are one of the central features of functional languages. As ordinary functions are abstractions over values with certain behavior, a higher-order function can receive functions as parameters and also produce a function~\parencite{hudak1989conception}. Thus functions are treated like any other value. This has convenient practical implications, where functions can be composed of other functions. For example, composing a function that executes a given function twice is possible:
\begin{lstlisting}
twice f = f . f
\end{lstlisting}

\noindent Now {\tt twice square 4}, where {\tt square} is {\tt square x = x * x}, would return 256. The function {\tt twice} is now~\textit{curried}, meaning that applying an argument to the function returns a function which also takes an argument. As {\tt twice} is a function it can be used again for function composition, et cetera. This kind of ``glueing'' together functions increases modularity of software written in a functional manner. Treating functions as first-class values also provides a way to abstract over functional behavior.~\parenciteseveral{hudak1989conception}.
%{{{
\begin{comment}
...
A convenient property of higher-order functions is their ability to generalize common behavior. For example, a function that ``folds'' over a data structure and returns a result value can be expressed as: 
\begin{lstlisting}
fold f z [] = z
fold f z (x:xs) = f x (fold f z xs)
\end{lstlisting}

The {\tt fold} function can then be used to accumulate values from a data structure, and the actual logic is passed on to {\tt fold} as a function:
\begin{lstlisting}
...
As can be seen, the actual data handling logic is separated from the data structure traversal. This leads into modular programs where components are likely to be reusable, thus resulting in fewer bugs and decreasing development time.

\section{Type systems}

Functional languages have traditionally taken extreme care of developing their~\textit{type systems}.~\citet[1.]{pierce2002types} provides a definition for type systems as 
\begin{quoting}
A type system is a tractable syntactic method for proving the absence of certain program behaviors by classifying phrases according to the kinds of values they compute.
\end{quoting}

\noindent Type systems and type theory is a vast field of study involving mathematics, logics and philosophy. Programming languages involve type systems by utilizing them in the inference, or calculations, of a program's run-time behavior. This reasoning can be done statically by a compiler or an interpreter, or dynamically at run-time. In essence, the type system is responsible for detecting and preventing errors in the execution flow of a running program through a type checker~\parencite{cardelli1996type}.

Two categories exist for the typing of programming languages. Most mainstream languages fall in to the category of~\textit{typed} languages, where program values are bound with information about their types~\parencite{cardelli1996type}.~\textit{Untyped} languages omit such information and contain at most one general type. Examples of untyped languages and systems, containing one type at most, are~\parencite{cardelli1985understanding}:

\begin{itemize}
    \item Assembly languages (bit patterns in memory)
    \item Pure LISP (S-expressions)
    \item $\lambda$-calculus ($\lambda$-expressions)
    \item Set theory
\end{itemize}

\noindent Types can be regarded as an aid for enforcing the correctness of software and limiting inconsistencies. Untyped languages, however, remove any restrictions on how different values and operations can interact together.~\parenciteseveral{cardelli1985understanding}. Developing with untyped and unsafe languages is thus error-prone, because no automatic help from e.g. the compiler is provided in case of logical inconsistencies~\parencite{cardelli1996type}.

Additionally, programming languages implement either~\textit{static checking} or~\textit{dynamic checking} (often called static and dynamic typing in practical settings)~\parencite{pierce2002types}. Static checking in languages such as Haskell and C is implemented at compile-time, where type information is derived by a combination of explicit annotations and type inference. Dynamically checked languages, such as Lisp, add run-time information about the program's values to validate behavior when it is executed.~\parenciteseveral{cardelli1996type}. Often both static and dynamic checking are utilized to provide more abstract language mechanisms such as Java's polymorphism.

A statically checked language has the benefit of being able to detect errors in the types of the program at compile-time, delivering errors sooner rather than later at run-time. Moreover, when the expressions of the program code have been determined as type-correct, no run-time checking needs to be done, leading to execution efficiency. However, static checking can also restrict expressiveness, since the types have to be known in advance.~\parenciteseveral{cardelli1985understanding}.

Most typed programming languages implement type checking by requiring explicit type annotations for functions and local variables. Subexpressions in the function do not require explicit type information, since the compiler can deduce it from the environment of the function.~\parenciteseveral{cardelli1985understanding}. Functional languages, however, have taken this type deduction even further.~\textit{Type inference} is utilized to provide accurate type information of expressions without requiring explicit annotations. This frees developers from having to repeat type declarations, but still enables the type system to support abstract language mechanisms such as polymorphism~\parencite{hudak1989conception}.

A dominant type system in functional programming is the Hindley-Milner (HM) type system~\parencite{milner1978theory}, where type inference plays an important role. HM can be implemented using Algorithm~\textit{W}~\parencite{damas1982principal} to effectively deduce the most general types in an expression without explicit type annotations. Indeed, the ML language, a major inspiration of Haskell, was the first statically typed language to integrate type inference into its implementation~\parencite{hudak1989conception}.

Haskell is considered to be a statically typed language with strong type inference. However, programming language type systems and type theory involve varying perspectives and terms, to which no unanimous consensus exists. To exaggerate the overlapping of different type concepts, a pure functional language such as Haskell also exhibits characteristics of dynamic checking and both implicit and explicit typing~\parencite{hudak2007history}.

\section{Strict and non-strict semantics}

Programming languages possess~\textit{strict} and~\textit{non-strict} semantics, which are determined by the evaluation strategy of functions used in the language. Similarly, lambda calculus utilizes~\textit{reduction strategies} that act as rules for evaluating lambda expressions. Most programming languages utilize strict semantics, where function arguments are eagerly evaluated. The paradigm of a language does not necessarily determine the evaluation strategy to be used. For example, both C (imperative) and Scheme (functional) languages utilize variants of the~\textit{call-by-value} strategy where the outermost expression is evaluated and ``only when its right-hand side has been reduced to a value''~\parencite[57.]{pierce2002types}.
%{{{
\begin{comment}
    ...
    the bound variable in the abstraction¿s body. Graphically, we write~\parencite{pierce2002types}
\end{comment}
%}}}

As lambda expressions are prevalent in the functional programming paradigm~\parencite{hudak1989conception}, the evaluation strategies are demonstrated with lambda reductions in this thesis. Perhaps the most utilized evaluation strategy, call-by-value, executes as follows:
...
\noindent The call-by-value strategy reduces redexes from the outermost first and only when its right-hand side has been evaluated. In practice the function arguments are always evaluated, regardless if they are needed or not. As a result, a programming language utilizing call-by-value is defined as strict.~\parencite{pierce2002types}. 

Non-strict evaluation strategies, however, have the benefit of not evaluating computations until needed. For example, in the~\textit{normal order} strategy, a lambda expression is reduced with the leftmost, outermost redex first~\parencite{pierce2002types}:
...
\noindent A language utilizing such an evaluation strategy is non-strict, because functions are applied before evaluating their arguments. However, the normal-order reduction is potentially inefficient when dealing with nontrivial computations, leading to unnecessary repetitive calculations in naive implementations~\parencite{hudak1989conception}. The~\textit{call-by-name} strategy, on the other hand, does not allow reductions inside abstractions, so the reduction
process stops one step earlier than in the normal-order example:
...
\noindent This has the advantage that computations are only evaluated when they are needed. Furthermore, Haskell utilizes an optimized, albeit a more complex version of this strategy, termed \textit{call-by-need}. In call-by-need, expressions once evaluated are saved and the values are reused without repeating the computations. This~\textit{lazy evaluation} is achieved through graph reduction, where arguments and their evaluated results are shared between computations~\parencite{hudak2007history}. 

A well-known property of lazy evaluation is the ability to declare unbounded data structures, such as infinite lists~\parencite{hudak1989conception}:
%{{{
\begin{comment}
...
\noindent The values of these lists are not computed until they are needed. For example, {\tt take 5 squares} would return {\tt [0, 1, 2, 4, 9]}. Effectively, lazy evaluation provides three important mechanisms for evaluating function parameters~\parencite{hudak2007history}:
...
\noindent As previously presented, functions in functional programming have the convenient property of being composable from other functions (warranted that their function signatures match). In conjuction with lazy evaluation, this function composition becomes especially useful when ``glueing'' several functions together. A popular usage in functional programs is to include ``generators'' (functions) that produce arbitrarily large data sets. However, combining these generators with functions that include predicates for
selecting the correct values provides a modular and concise way to structure applications. This separation of data structures and control is also one of the central notions of functional programming.~\parenciteseveral{hudak1989conception}.
%{{{
\begin{comment}
...
%{{{ Additional sources
\begin{comment}
    By abstracting away from evaluation order, lazy evaluation also
    abstracts away from object lifetimes, and that is why lazy evalua-
    tion contributes to space leaks. Programmers who cannot predict¿
    and indeed do not think about¿evaluation order also cannot pre-
    dict which data-structures will live for a long time. Since Haskell
    programs allocate objects very fast, if large numbers of them end
    up with long lifetimes, then the peak space requirements can be
    very high indeed. The next step was thus to extend the heap profiler
    to provide direct information about object lifetimes.~\parencite{hudak2007history}
...
Programming languages can be defined as having either~\textit{monomorphic} types, where only one type exists for functions and operands, or~\textit{polymorphic} types, where values can have numerous types. For example, a polymorphic function might have arguments that, in turn, can be represented as more than one type. Additionally, these polymorphic types may have operations that are applicable to multiple types.~\parenciteseveral{cardelli1985understanding}.

Polymorphism is further divided into more detailed concepts.~\textit{Parametric} polymorphism enables functions to operate on a uniform set of unconstrained types that share a common structure~\parencite{cardelli1985understanding}. For example, the idiomatic function {\tt id :: a -> a} generally accepts any kind of type as its argument and as a result, returns a value of the same unknown type.~\textit{Ad-hoc} polymorphism, on the other hand, enables
functions to operate on seemingly unrelated types with an implementation that is specific to the type~\parencite{cardelli1985understanding}. The addition operator {\tt +} implements ad-hoc polymorphism in many languages, since it operates differently based on its input types. For example, adding two strings together might result in a concatenated string, where as adding two decimal numbers might utilize a floating point operation to calculate the result. In fact, the types have~\textit{overloaded} the implementation of the addition operator. 

The data abstraction mechanisms of modern functional languages are elaborate. The flexibility of functional programs is increased by~\textit{algebraic data types}, which enable a computation to depend on type alternatives. This means that the data types of functional languages are often~\textit{polymorphic}, i.e.\ they can represent multiple types. An algebraic type can be generalized as ``a sum of one or more alternatives, where each alternative is a product of zero or more
fields''~\parencite[15.]{hudak2007history}. An example of algebraic types is to represent the types of a binary search tree:
%{{{
\begin{comment}
...
Effectively, {\tt size} receives a data type of {\tt Tree} as the only parameter. In fact, the function signature is {\tt size :: Tree a -> Int}. The pattern matching mechanism is then utilized for evaluating either expression of {\tt size}, depending on the actual data constructor. 

Due to the nature of lazy evaluation, the actual implementation of pattern matching has proved to be challenging in functional languages. In particular, the ordering of parameters rise subtle problems depending on their evaluation mechanism~\parencite{hudak1989conception}. In the end, matching patterns from top to bottom and evaluating parameters from left to right serves as a sufficient compromise and is used in most modern functional languages~\parencite{hudak2007history}.
%{{{
\begin{comment}
...
~\textit{Abstract data types} address the need for increased modularity and security. Specifically, the actual data type is not exposed outside its module, but rather, can be accessed through utility functions. Similar to~\textit{interfaces} in Java, abstract data types allow changing the representation type without affecting external modules that depend on this ``interface''.~\parenciteseveral{hudak1989conception}. The {\tt Tree} data type is abstract, because it does not restrict the
actual values it can contain. The type variable {\tt a} can represent integers, strings or any other data type. Thus modularity is increased, because the implementation is abstracted away. On the other hand, security is enhanced because no unprivileged calls are allowed by the abstract data type's interface.
%{{{
\begin{comment}
...
Together with a strong type system and data types a functional language such as Haskell aids the developer in the creative and also in the problem-solving process of programming. Algebraic and abstract data types allow conciseness and clarity in program code, and strong static type checking helps in pinpointing faulty code~\parencite{hudak1989conception}.

\section{Functors}

Functors add flexibility when utilizing different data types, such as the previously presented {\tt Tree} construct or an ordinary list data structure. As looping over the values of a construct would be convenient, functors aim at providing an abstraction for this kind of behavior. Namely, functors enable ``mapping'' functions over data types and is one of the central routines of functional programming.~\parenciteseveral{lipovavca2011learn}. Functors can be thought as
...
Functional programming languages can roughly be divided into two categories;~\textit{pure} and~\textit{impure} approaches. Pure functional languages, such as Haskell, utilize lambda calculus in its simplest form, while impure languages, such as Scheme, introduce additional features, such as assignments, into computations. The main motivation behind ``pure'' languages is that they make data flow explicit and expressions only depend on their free variables. This enables a powerful aid in reasoning about the code's logic, since substitution can always be done in-place. On the other hand, impure features in a language can offer more conveniency for the developer as the concept of purity is not strictly enforced.~\parenciteseveral{wadler1995monads}.
%{{{
\begin{comment}
...
As an example, adding error handling to program logic is often tedious, since every recursive function call is to be checked for errors and subsequently handled. In contrast, exceptions in an impure language would not introduce such structuring of code. More to the point, counting the number of operations performed on a function require similar structuring of the code where the count is explicitly passed on for further operations. Again, in an impure language a global variable for the count would suffice.~\parenciteseveral{wadler1995monads}. Largely for this division of pure and impure functional languages,~\textit{monads} were introduced as an utility to combine the benefits of both approaches. Namely, monads are used ``to integrate impure effects into pure functional languages''~\parencite[25.]{wadler1995monads}. As is customary for the functional programming paradigm, also the concept of monads is derived from mathematics, and in this case, category theory.~\parenciteseveral{wadler1995monads}.
%{{{
\begin{comment}
...
Monads can be seen as ``wrappers'' or contexts for values, similar to functors and applicative functors. In Haskell, monads are often used for handling state, exceptions, output and for nondeterministic behavior.  Indeed, I/O operations are specifically structured as monads in Haskell and special syntax is reserved for expressing them~\parencite{typeclassopedia2014haskellwiki}.

Even if monads are perhaps the more popular topic of functional programming, they are not any more complex from other constructs of computational contexts. For example, a developer wishes to operate on a data structure, but also keep a reference on how many operations were applied. In a pure approach, the obvious solution to increment the counter would be to pass it as a parameter to all subsequent iterations. Monads, however, can be utilized in this scenario to represent the counter variable.
...
Functional programming is a distinct paradigm compared to the arguably more widespread imperative and object-oriented approaches. However, functional languages attempt to provide a more mathematical structure to programming, which in turn, is argued to lead to better reasoning of the program logic. Referential transparency, where expressions can be evaluated and their variables replaced by values at any time, is perhaps the most convincing aspect of functional programming~\parencite{hughes1989functional}.   

Lambda calculus is a prevalent system for expressing computations in functional programs, and in its simplest form only contains few constructs (redexes, abstractions) and is relatively simple to be applied through variable binding and substitution. Haskell utilizes the more complex call-by-need strategy to achieve the well-known attribute of lazy evaluation~\parencite{maraist1998call}.

The expressiveness of functional programs are increased by their use of higher-order functions and strong type systems. Program logic often consists of short and succinct functions that can be combined together through currying. Furthermore, the parameters of functions are often (custom) type variables, which are extensively supported by e.g. Haskell's type system.~\parenciteseveral{hudak1989conception}.

The fascinating aspect of functional programming is its basis on mathematical notions. Through this connection Haskell, widely popular in academia, continues to be researched and as a result new approaches (see e.g.~\citet{baars2004functional,mcbride2008functional}) are found at regular intervals. For example, the popular construct of monad was not initially included in the design of Haskell~\parencite{hudak2007history}. In fact, the usefulness of monads in programming language design
was not realized until the early 90s~\parencite{moggi1991notions}.

This chapter served a basis for functional programming and further established motivation for the significance of approaches where reasoning logic and expressiveness are held central to the programming effort. Moreover, this thesis can not contain but a limited introduction to functional programming (and Haskell), but readers are nevertheless encouraged to learn a new programming paradigm (see~\citet{lipovavca2011learn,o2008real} for learning Haskell).

\chapter{Metrics for functional programming}
\label{chap:metrics_for_functional_programming}
...
Prior research on metrics exclusively applicable to functional programming is scarce. Foundings of such research are mostly laid by~\citet{van1995software} and~\citet{ryder2004software}, whose metrics are primarily discussed in this section. The initial work of~\citet{van1995software} was born because of the need to effectively measure the readability of code produced by students taking part in functional programming courses. Somewhat fortunately, many of the existing metrics, such as
...
The validation methodology used by~\citet{ryder2004software} is based on comparing version change histories of Haskell programs to the values output from the metrics presented. A high metric value from a program module is further assessed by~\textit{subjective complexity}, which is referred as ``a notion of the complexity that might be perceived by a programmer attempting to understand or modify the given function''~\parencite[103.]{ryder2004software}. Subjective complexity is, of course,
affected by many variables such as the experience of the programmer and cannot be ``concretely specified''~\parencite[103.]{ryder2004software}. As a result,~\citet{ryder2004software} uses subjective complexity as an aid for determining the correctness of the metric values.

\section{Complexity of patterns}

One of the most used features in functional programming is pattern matching. In Haskell, pattern matching is used in various ways (see section~\ref{sec:polymorphic-types} for an example). Furthermore, the {\tt \_} wildcard can be used to match (or ignore) arguments, and patterns can also be used in function bodies. Constructing metrics to deduce the complexity of patterns is then an obvious first interesting step towards measuring FP programs.
...
The~\textit{number of constructors} in a pattern can be an indicator of program complexity. Two hypotheses are provided by~\citet{ryder2004software}, where the first deals with the notion that an increased number of constructors also increase complexity. The second hypothesis argues, counteractively, that if the constructors are named properly, it can reduce subjective complexity of a function. Once again,~\citet{ryder2004software} found a slight statistical correlation
...
\noindent Using the two case-study programs,~\citet{ryder2004software} found correlation between complexity and increased number of wildcards in pattern matching. However, he further argues that the complexity is not necessary due to wildcards themselves, but because of the more complex data structures involved. Again, exhibiting many function arguments that are not encapsulated to data types is a sign of the need to refactor the program.
%{{{
\begin{comment}
...
For both patterns in listing~\ref{lst:depth-of-nesting}, {\tt pattern1} and {\tt pattern2}, the maximum depth is 2. However, sums of the depths are not the same between the patterns. Finally,~\citet{ryder2004software} found a somewhat strong correlation for the value of increased depth of nesting and subjective complexity.
%{{{
\begin{comment}
...
\section{Distance between declarations and usage}
...
\section{Attributes of recursive functions}
...
First recursion measure provided by~\citet{ryder2004software} is the~\textit{binary indication of recursion}. The metric simply determines if a function is recursive in nature, indicated by a value of zero or one. This seemingly simple metric can correlate with the number of changes in the code base, and thus may indicate increased subjective complexity. Non-trivial recursion is when a function invokes a function, which in turn invokes the original function back. This kind of
...
Often recursion is invoked based on multiple paths in a function. The metric~\textit{number of recursive paths} determines the number of paths leading to recursion in a function, which is argued to correlate with subjective complexity. 
...
Counting the~\textit{number of non-trivial and trivial recursion} can indicate complexity in the code base. As presented previously, trivial recursion occurs when a function invokes itself, as where non-trivial recursion deals with two functions that invoke each other. If the number of non-trivial recursion exceeds trivial recursion, it can be hypothesised that the complexity of the code also increases. Again,~\citet{ryder2004software} found no support for the hypothesis,
...
Lastly, all of the recursion-specific metrics are likely to interact with each other, and as such, observation of their interaction from a measurement perspective is worthwhile to conduct. It can be argued, however, that the abundant use of explicit recursion in functional programs is against good practice and should signal immediate refactoring efforts. 

\section{Attributes of call graphs}

As presented earlier in section~\ref{section:data-flow-attributes}, call graphs are a convenient way of rendering and analyzing the data flow of a program. As functions are the main tool of functional languages, it should be possible to utilize much of the same metrics of call graphs to the functional paradigm.
...
\section{Miscellaneous attributes of functions}
...
\section{Metrics for higher-order functions}
...
\begin{itemize}
        \item Ratio of the number of functions in a module that have at least one external function as their parameter and the number of functions in the module with a function as a parameter.  
        \item Ratio of the number of external functions that are called in a first-order way from the current module and the number of functions outside the module
...
Software metrics studies specific to the functional programming paradigm are scarce.~\citet{van1995software} is the first to research the topic in-depth~\parencite{berg1992syntactic,van1995static}, and extensive work by~\citet{ryder2004software,ryder2005software} furthermore increase the body of research. New programming languages such as Scala brought about new interest in functional programming and metrics study is likewise revitalized~\parencite{muddu2013metrics}.

As is with the more ``traditional'' metrics visited in chapter~\ref{chap:software_metrics}, also the functional metrics exhibit strong dependancy on subjective observations, rather than systematic and evidence-based outcomes. In essence, many of the metrics are, in fact, subjective by nature, since e.g. readability is hard to measure objectively. However, the problem of lacking evidence is magnified in the functional paradigm metrics studies due to sheer low amount of research conducted.

The positive side is that the functional paradigm seems to be well-suited for software measurement. Metrics such as~\textit{number of constructors} and~\textit{number of overridden pattern variables}~\parencite{ryder2004software} are easy to quantify and show promising preliminary results. Call graphs are convenient for visual comprehension of the program logic flow, but arguably even more so for functional programs where
functions are numerous. In addition, call graphs can quickly visualize the overall ``health'' of the system. Indeed, the findings of~\citet{ryder2004software} suggest that visual measures are suitable for capturing software complexity.

Complexity between modules can be measured with metrics such as~\textit{intermodule referential transparency index} or~\textit{intermodule first order functions index}. Such metrics are interesting for analyzing the overall quality of the system, rather than individual functions. Indeed,~\citet{muddu2013metrics} present promising results of the degree of modularization between large open-source projects' versions.

Even though the foundations for metrics research in the functional paradigm are laid out well, the quantity of studies leaves much to hope for. In part this may be because of the functional paradigm's apparent overshadowing by the imperative and object-oriented paradigms used in the industry. On the other hand, languages such as Scala~\parencite{odersky2004overview} have brought about new interest in the functional paradigm for commercialized use, increasing metrics research
activity in the last couple of years~\parencite{muddu2013metrics}. As the shift towards higher-level languages (and abstractions) in large-scale architectures increase, also the metrics are bound to adapt. Software metrics research seems to require a certain demand from the industry in order to thrive, however.  
...
The aim of this study was to explore the current status of software metrics research together with functional programming. In specific, the objective is to present previous research on software metrics, introduce functional programming as a programming language paradigm and to assess the depth and breadth of software metrics research specific to the functional paradigm. The topics chosen are relevant in many ways - measuring software is as old as computer science itself and
proactively pinpointing problematic program components ultimately reduce production costs. Functional programming is likewise an old topic, but has eluded wide mainstream use despite no apparent weaknesses. Combining the two topics is thus relevant for exploring future ways of conducting software engineering and paving way for new research.

The research method chosen for the thesis - literature review - is fitting, since the subject is variably researched~\parencite{kitchenham2010s,ryder2005software} and conducting reviews is an excellent way of mapping the current situation and generating interest in to the subject. The results of the literature review are promising. Withstanding the lack of studies in the early 80s and low activity in the 00s, software metrics research has shown credible activity during the recent years. Moreover, interest in functional programming is rising, which is evident from new studies measuring complexity of functional programs (see e.g.~\citet{muddu2013metrics}). On the other hand, much of the
measuring techniques have shifted towards prediction models~\parencite{radjenovic2013software}, where targets of component maintenance are calculated probabilistically, e.g.\ based on software change history~\parencite{graves2000predicting}.

As predicted, the agile software engineering~\parencite{beck2001agile} approach has dramatically decreased the interest in software measurement, because it can be seen as unnecessary work not suitable for the agile workflow. Moreover, quantifiable approaches (which measurement can be considered as) are not advocated in the agile manifesto, which specifically values ``individuals and interactions over processes and tools''. As it is, software measurement and the agile
workflow have not yet been combined together to provide a flexible, but quantifiable approach to software engineering.

In conjuction with the findings of~\citet{kitchenham2010s}, this thesis recognizes the vast amount of research done in the field. This is, of course, favourable, but complicates the process of navigating through the studies. As~\citet{kitchenham2010s} points out, the research methods are almost as numerous as the number of studies. Unified research frameworks have been proposed~\parencite{kitchenham1995towards,briand1996property,habra2008framework} but none have been
widely adopted for unknown reasons. Furthermore, too many metrics proposed lack industrial validation, which, in the end, is the target audience if their use is to be adopted.

The main research topic of the thesis was to investigate how software metrics apply to the functional programming paradigm. As evident from the previous chapter, the basis has been established by~\citet{van1995software,ryder2004software} and newer research from~\citet{muddu2013metrics} seem promising. The metrics proposed are plausible from both scientific and practical viewpoints. They are well-formed and, by the evaluation of this thesis, are measuring language
constructs in a reasonable way (such as the number of
pattern variables). Reproducibility also seems to be taken into account, since the research methods are clearly explained and should pose no restrictions for replicating these studies.

The results are that even if functional programming is touted as being superior in readability and expressiveness than imperative programs, the functional paradigm can be enhanced by utilizing software metrics. The fact is that the functional programming paradigm lacks widespread use, at least compared to the imperative and object-oriented approaches. This alone limits the comparability of the functional paradigm and its metrics against other approaches.

The significance of the results are moderate. The functional paradigm is gaining traction also in practical settings, and the crossing of software metrics into that area is intriguing. At the very least, this thesis adds mass to the body of studies, which in its current state leaves much to hope for. The impact of the thesis' results on future research is harder to predict. As software metrics research seems to be revitalizing, also the need for review and mapping
studies is increased.

Future research is oriented towards deeper understanding of software metrics in functional programming. More empiric research with sound measuring frameworks is needed to assure the audience that software metrics are effective in certain settings. As the size of information systems keeps growing, unfortunately also their complexity is increased and likely with more than linear progression. Software metrics are one candidate for containing this inevitable complexity, but they still are somewhat heavy to implement and thereby not favoured by the agile software development community. As an important future research topic, this thesis calls out for the need to merge software measurement into current software development practices. Even in moderate-sized projects, an easy-to-use and accurate measuring tool could cut down on project costs and development time.
%}}}
