...
Since monads are derived from category theory~\parencite{moggi1991notions}, specific laws exist for validating whether a construct represents a monad.  
...
Functional programming is a distinct paradigm from the arguably more wide-spread imperative approaches like C~\parencite{kernighan1988c}. However, functional languages attempt to give a more mathematical structure to programming, which in turn, is argued to lead to better reasoning of the program logic. Referential transparency, where expressions can be evaluated and their variables replaced by values at any time, is perhaps the most convincing aspect of functional programming~\parencite{hughes1989functional}.   
...
The fascinating aspect of functional programming is its basis on mathematical notions. Through this connection, e.g. Haskell, widely popular in academia, continues to be researched and as a result new approaches (see e.g.~\citet{baars2004functional,mcbride2008functional}) are found at regular intervals. For example, the popular construct of monad was not initially included in the design of Haskell~\parencite{hudak2007history}. In fact, the usefulness of monads in programming language design
was not realized until the early 90s~\parencite{moggi1991notions}.

This chapter served a basis for functional programming and further established motivation for the significance of approaches where reasoning logic and expressiveness are held as a central programming construct. Moreover, this thesis can not contain but a limited introduction to functional programming (and Haskell), but readers are nevertheless encouraged to learn a new programming paradigm
(see~\citet{lipovavca2011learn,o2008real} for learning Haskell).
...
The research method chosen for the purposes of the thesis is arguably well-suited for scanning and aggregating results from a large body of research. The literature review approach lends itself sufficiently to the field of software measurement, where periodic reviews of past work are frequent and new areas of interest are generated. 
