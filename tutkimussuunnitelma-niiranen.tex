...
Traditionally, however, software metrics have mostly dealt with imperative and object-oriented programming paradigms, and as such, the metrics are tailored for them~\parencite{harrison1996evaluation,basili1996validation,li1993object,chidamber1994metrics}. As the use of functional (reactive) programming increases, also the analysis of such programs is increasingly required. This, in turn, has called out researchers to update and add software metrics that also fit to the functional paradigm concepts~\parencite{ryder2005software,harrison1995estimating}.

\section{Motivation}

Even though FRP is not a new programming paradigm~\parencite{elliott1997functional}, it has eluded wide adoption. Recently, however, more software practitioners~\parencite{netflix2013,futurice2014,flowdock2013} have started to implement both systems as well as graphical user interfaces in the spirit of FRP. Unlike software quality assessment on imperative and object-oriented languages, similar comparison on functional languages is scarce~\parencite{harrison1995estimating,harrison1996comparing,ryder2005software,ryder2004software,van1995software} and almost non-existant on the FRP paradigm. However, as the FRP paradigm gains popularity, also the software quality metrics should be updated to support the high-level abstractions of the approach.
...
\begin{itemize}
    ...
    \item How do software quality metrics relate to functional and reactive programming? 
    \item How do existing software metrics apply to FRP and what kind of FRP-specific metrics can be formulated?
\end{itemize}
...
The thesis utilizes literature review as its main research method. The review consists of an overview into functional and reactive programming and the metrics of software quality. In preparation of the empirical part, FP metrics~\parencite{ryder2004software} are scrutinized and FRP-tailored metrics are formulated.

In the second part, the research themes are investigated through empirical research. 

The source code of existing applications implemented with FP and FRP is analyzed and software quality metrics are quantified. The applicability of metrics and their quality are further analyzed based on the results.

As an alternative a tool for visualizing software metrics on FRP code bases could be developed.

\section{Expected results and their significance}
The results aim at explaining the various semantics of FRP and how they relate to software quality metrics. As FRP is especially tailored for data-driven designs, the results are expected to be positive in such settings. However, analyzing software implemented with FRP might differ from traditional software quality metrics. As some of the traditional software metrics are applicable to the FP paradigm~\parencite{ryder2004software}, also some of the existings metrics are assumed to apply to FRP
code bases too.

However, specific software metrics for FRP applications are likely to be needed. For example, the amount of Behaviors and Events~\parencite{elliott1997functional} and their coupling could serve as a simple metric for the FRP paradigm.
...
\begin{itemize}
    ...
    \item Pure functions (Side effect -free)
...
\begin{itemize}
    ...
    \item Haskell 98 language and libraries: the revised report~\parencite{jones2003haskell}
    \item Software Measurement for Functional Programming~\parencite{ryder2004software}
\end{itemize}
...
This chapter will provide an overview into software quality and the metrics of code quality analysis. Code metrics for imperative, object-oriented and functional languages are presented.
...
\begin{itemize}
    ...
    \item Software metrics: successes, failures and new directions~\parencite{fenton1999software}
    \item Implementing effective software metrics programs~\parencite{hall1997implementing}
    \item A complexity measure~\parencite{mccabe1976complexity}
    \item Software Measurement for Functional Programming~\parencite{ryder2004software}
    \item Software measurement and functional programming~\parencite{van1995software}
    \item Software structure metrics based on information flow~\parencite{henry1981software}
\end{itemize}

\subsection{Metrics}
\begin{itemize}
    \item Internal attributes~\parencite{hall1997implementing,fenton1999software}
    \item External attributes~\parencite{hall1997implementing,fenton1999software}
    \item Control flow
    \item Cyclomatic complexity~\parencite{mccabe1976complexity}
    \item Test coverage
    \item Coupling
    \item Fan-in \& Fan-out~\parencite{henry1981software}
\end{itemize}

\subsection{Metrics for functional languages}

\subsection{Metrics for functional reactive programming}

As no software metrics for FRP exists, this section analyzes the applicability of existing metrics to the FRP paradigm. Furthermore, software metrics specifically tailored for FRP are presented.

\subsection{Summary}

\section{Empirical study}

The thesis contains an empirical study in the form of application code review and metric analysis. Existing FP and FRP implementations will be analyzed and quantified with software metrics presented in the literature review.
